...
- `github_outbound` adds the public `asset_id` and `release_tag` only for an exact supported repository/asset URL with a bounded stable-version label. Old tabs may report older versions. These client-reported labels do not authenticate publication; this is a click, not a completed download or installation.
