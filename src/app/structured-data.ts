...
const englishSoftwareApplicationFeatures = [
  ...
  "Desktop app: a Tauri 2 + React 19 source-cited wiki GUI in github.com/7xuanlu/wenlan/tree/v0.18.3/app (AGPL-3.0-only). The current release ships a notarized macOS Apple Silicon DMG and Windows x64 setup executable. It talks to the same local daemon over HTTP at localhost:7878, is version-locked to the Wenlan workspace, and ships through the unified release manifest. It also provides guided Remote Access for ChatGPT and Claude.ai. Remote Access uses --no-auth behind a temporary HTTPS tunnel, so possession of the URL grants access.",
